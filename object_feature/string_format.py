...
format(*args,**kwargs)  #返回一个格式化后的字符串,原字符串的替换字段被适当格式化后的参数所替代.
    ...
    s="Hello,{0},{1},{2}"
    s.format("Java","Python","Netty")
    ->'Hello,Java,Python,Netty'


    如果字段名是字符串(不能以数字开头,或者是数字),那么format()方法中将使用的格式:字段名=值

    s="Hello,{A1},{A2},{A3}"
    s.format(A1="Java",A2="Python",A3="Netty")
    ->'Hello,Java,Python,Netty'
    ...
    s="Hello,{name},{1},{0}"
    s.format("Python","Netty",name="Java")
    ->'Hello,Java,Python,Netty'
...
参数解析:
    ...
    align: "<"left,">"right,"^"center对齐方式,
    "="用于在符号与数字之间进行填充
    sign: "+"|"",数字是否带正负标识(+表示正数带符号标识,
    ""标识正数带空白标识)
    #:整数前缀为0b,0o,0x
    width:最小宽度
...
字符串格式规约是使用冒号(:)引入的,其后跟随可选的字符--一个填充字符(也可没有)与一个对齐字符
(<用于左对齐,>用于右对齐,^用于中间对齐),之后跟随的是可选的最小宽度(整数),若需要指定最大宽度,
就在其后使用句点,句点后跟随一个整数.
...
填充:数字或者字符串包含对齐方式的长度达不到width时候,会发送在字符串前填充达到最小宽度
    ...
    '{0:0=12}'.format(1234)或者'{0:012}'.format(1234)
    -->'000000001234'
...
格式化的是字符串或者浮点数:precision指明输出字符串的最大长度,对于超过的部分截断.(小数点不计入长度当中)
'{0:1.2}'.format('abc')-->'ab'
