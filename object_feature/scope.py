...
名称的绑定(确定名称作用域)
    ...
    下面结构将会绑定名称：
        ...
        3.类和函数定义(绑定类或函数的名称于定义它们的代码块中):类和函数定义将类名和函数名作为变量引入当前作用域,类体和函数体将形成
    另外一个作用域;
...
名称的解析(确定运行时名称的值)
    ...
    异常:
        ...
        如果当前的作用域是一个函数作用域,而且名称引用一个局部变量,这个变量在该名称使用的时候还没有绑定到一个值,则引发一个
    UnboundLocalError异常(UnboundLocalError是NameError的子类).
    ...
    global的运行机制和作用域规则:
        ...
        global语句的作用域与同一代码块中的名称绑定操作相同(局部范围可以操作上层的变量).如果包含自由变量的最内层定义域包含一条global
        语句,那么这个自由变量[声明式变量]被认为是一个全局变量.
