...
数据共享与同步: 通常,进程之间彼此是完全孤立的,唯一的通信方式是队列或管道. 但可以使用两个对象来表示共享数据.
    ...
    Value(typecode, arg1, arg2, ... argN, lock)  在共享内存中创建ctypes对象. typecode要么包含array模块使用的相同类型代码(如
        'i', 'd'等)的字符串,要么是来自ctypes模块的类型对象(如ctypes.c_int, ctypes.c_double等). 所有额外的位置参数arg1, arg2,
        将传递给指定类型的构造函数. lock是只能使用关键字调用的参数,如果是True(默认值),将创建一个新锁来保护对值的访问. 如果传入一个
        现有锁,如Lock或RLock实例,该锁将用于进行同步. 如果v是Value创建的共享值的实例,便可以使用v.value访问底层的值. eg: x=v.value
        v.value=xx

    Array(typecode, initializer [, lock]) 在共享内存当中创建ctypes数组. typecode描述了数组的内容,意义与Value当中的相同.
        initializer要么是设置数组初始大小的整数,要么是项序列,其值和大小用于初始化数组. lock同Value当中的含义. 可以使用标准的
        Python索引,切片,迭代操作访问其内容,每种操作均由锁进行同步. 对应字节字符串,具备a.value访问.
