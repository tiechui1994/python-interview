"""
array模块:
    定义了一个新对象类型array.该类型工作原理与列表及其相似,但是它的内容仅限于单一类型.数组的类型在创建数组时
    确定.
        类型编码    描述          C类型         最小大小(字节)
        'c'        8位字符        Char              1
        ...
        方法:
            ...
            count(x)
            extend(b)    数组合并
            fromfile(f,n) 从文件对象f中读取n个项(二进制格式),并追加到数组末尾.f必须是一个文件对象,且可读
                取的项大于n
...
数据共享与同步: 通常,进程之间彼此是完全孤立的,唯一的通信方式是队列或管道.但可以使用两个对象来表示共享数据.
    ...
    Value(typecode,arg1,arg2,...argN,lock)  在共享内存中创建ctypes对象.typecode要么包含array模块使用
        的相同类型代码(如'i','d'等)的字符串,要么是来自ctypes模块的类型对象(如ctypes.c_int,ctypes.c_double
        等).所有额外的位置参数arg1,arg2,将传递给指定类型的构造函数.lock是只能使用关键字调用的参数,如果是True
        (默认值),将创建一个新锁来保护对值的访问.如果传入一个现有锁,如Lock或RLock实例,该锁将用于进行同步.如果
        v是Value创建的共享值的实例,便可以使用v.value访问底层的值.eg: x=v.value v.value=xx

    Array(typecode,initializer [,lock]) 在共享内存当中创建ctypes数组.typecode描述了数组的内容,意义与
        Value当中的相同.initializer要么是设置数组初始大小的整数,要么是项序列,其值和大小用于初始化数组.
        lock同Value当中的含义.可以使用标准的Python索引,切片,迭代操作访问其内容,每种操作均由锁进行同步.
        对应字节字符串,具备a.value访问.
    ...
    注意: 使用多进程后,不需要担心与锁,信号量或类似构造的底层同步.在某种程度上,管道的send()和receive(),以及
        队列的put()和get()已经提供了同步功能.但是,在使用数据共享(Value,Array)的时候,需要使用锁,信号量等机
        制进行同步操作.
"""
