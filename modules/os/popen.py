...
"""
Popen(args,**argv) 以子进程形式执行一个新命令,然后返回代表新进程的Popen对象.命令在args中指定,是字符串
    ('ls -l')或字符串列表(['ls','-l']).argv表示关键字参数的集合,设置这些参数可控制子进程的各种属性.
    ...
    stderr 这个文件对象代表stderr在子进程中使用的文件.可能是通过open()函数创建的文件对象,整数的文件描述符,
    或者特殊值
...
p.communicate([input]) 通过将input中提供的数据发送给进程的标准输入,与子进程进行通信.数据一旦发出,方法就会等
    待进程终止,同时收集标准输出上接收到的输出和标准错误.返回值是一个元组(stdout,stderr)
