...
进程环境:
    ...
    getpid() 当前进程的ID
    getppid() 当进程的父进程ID
    getpgid(pid) 当前pid进程的进程组ID  => os.setpgid(pid,pgrp) 将进程pid赋给进程组pgrp.
        若pid == pgrp进程将成为一个新的进程组主进程.
    ...
    getenv(name,default)
    putenv(name,value) 将环境变量name设为value.修改将影响到以os.system(),popen(),fork(),execv()
        开始的子进程.给os.environ中的赋值将自动调用putenv().但是调用putenv()不会更新os.enviro
...
文件创建与文件描述符:
    ...
    dup(fd) 复制文件描述符,返回新的文件描述符,它是进程未使用的文件描述符中编号最小的描述符.新的和旧的文件描
        述符可以交换使用.此外,它们还共享状态.
    dup2(oldfd,newfd) 将文件描述符oldfd赋给newfd.如果newfd已经对应一个有效的文件描述符,那么它将被首先关
        闭.
    ...
    fdsync(fd) 强制将fd中已写入的内容写入磁盘.注意:如果使用不带缓存IO的对象(file对象),那么在调用fsync()函
        数之前需要先清除数据.

    ftruncate(fd,length) 截取对应文件描述符fd的文件.从而让该文件的大小最多不超过length个字节(Unix)

    lseek(fd,pos,how) 将文件描述符fd的当前位置设置为pos.how的取值如下: SEEK_SET用于设置相对文件的开始处
        的位置.SEEK_CUR用于设置相对当前文件描述符的位置.SEEK_END用于设置相对文件结尾的位置.
    ...
    pipe() 创建一个管道,可用于与另外一进程建立单向通信.返回一对文件描述符(r,w),分别用于读取和写入.此函数同城
        在执行fork()函数之前调用.在fork()函数之后,发送进程将关闭管道的读取端,而接收进程将关闭管道的写入端.
        至此管道被激活,可以使用read()和write()函数将数据从一个进程发送到另外一个进程(UNIX)
...
"""
文件与目录:
    access(path,accessmode) 检查此进程访问文件path的读取/写入/执行的权限.R_OK(读取),W_OK(写入),X_OK
        (执行),F_OK(存在).访问得到授权返回1,否则返回0

    chflags(path,flags)  修改path上的文件标志.以UF_开头的标志可以由任何用户设置,以SF_开头的标志只能是超
        级用户才能修改.
    ...
    listdir(path) 返回包含目录路径中各项名称的列表.如果路径是Unicode编码的,结果列表将只包含Unicode字符串.
        如果路径是字节字符串,那么所有的文件名都以字节字符串列表的形式返回.
    ...
    mknod(path,modem device) 创建特殊文件.mode是文件的权限和类型.而device是使用os.makedev()函数创建的
        原始设备编号.设置文件的访问权限时,mode参数接收的参数与open()相同.
    ...
    renames(old,new) 递归的目录重命名和文件重命名函数.首先会尝试创建命名路径所需的中间目录,重命名之后,将
        使用removedirs()函数删除对应于旧名称最右边路径部分的目录.

    utime(path,(atime,mtime)) 设置文件的访问时间和修改时间.时间参数使用time.time()函数返回的数字来指定.

    walk(top,topdown,oneerror,followlinks) 创建一个生成器对象来遍历整棵目录树.top是指定目录的顶级,
        topdown是一个布尔值,用于指示是由上而下(默认)还是由下而上来遍历目录.返回的生成器将生成元组
        (dirpath,dirnames,filenames); dirpath是一个字符串,包含通向目录的路径,dirnames是dirpath中所有
        的子目录; filenames是dirpath中文件的一个列表.oneerror是一个接收单参数的函数.默认该函数什么也不做.
"""
...
进程管理:
    ...
    execvp(path,args) 很像execv(path,args),但在目录列表中搜索可执行文件的过程中将复制shell的动作.字典
        列表从enviro['PATH']中获取.
    ...
    _exit(n) 使用状态n立即退回到系统,同时不执行任何清理动作.这同城只在由fork()函数创建的子进程中完成.这与
        调用sys.exit()不相同,sys.exit()函数会正常关闭解释器.退出代码n依赖于应用程序.
        EX_OK 没有错误
        EX_USAGE 错误的命令用法
        ...

    fork() 创建一个子进程.在新创建的子进程中返回0,在原始进程中返回子进程的进程ID.子进程是原始进程的克隆,它
        们共享众多资源.
    forkpty() 创建一个子进程,并使用一台新的伪终端作为子进程的控制终端.返回值(pid,fd) 其中pid在子进程中为0,
        而fd是伪终端的主端的文件描述符.(UNIX)

    kill(pid,sig) 发送信号sig到进程pid.signal模块可以查找信号名称的列表
    killpg(pgid,sig) 发送信号sig到进程组pgid

    nice(increment) 提高进程的调度优先级("niceness").返回一个新的niceness.用户一般只能降低进程的优先级,
        只有root权限才可以提高优先级

    plock(op) 将程序段锁在内存中,从而防止它们被交换.op的值是一个整数.
    popen(command,mode,bufsize) 给名称打开一条管道.返回值是连接到管道的打开文件对象.根据模式是'r'(默认),
        还是'w',读入或写入这个文件对象.命令的退出状态由返回的文件对象的close()方法返回,如果退出状态是0,则
        返回None

    spawnv(mode,path,args) 在新进程中执行程序,以命令行参数的形式传递args中指定的参数.args可以是列表或元组.
        args的第一个元素是程序的名称.mode可以是以下值:
        P_WAIT  执行程序并等待它终止.
    ...
    startfile(path,operation) 运行文件和path相关的应用程序.这样执行的动作在Windows Explorer中双击文件相同.
        在应用程序运行之后,函数就返回.此外不能等待完成或者从应用程序活动退出代码.path的值是相对当前目录而言.
        operation是一个可选的字符串,用于指定打开path时执行的动作,默认值是'open','prinit','edit',explore',
        'find'
    
    system(command) 在子shell中执行command(字符串).在UNIX上,返回值和wait()函数一样是进程的退出状态.

    wait([pid]) 等待一个子进程完成并返回包含其进程ID和退出状态的元组.退出状态是一个16位数字,其低位字节是终止
        进程的信号编号,而高位字节则是退出状态(如果信号编号是0).如果生成了核心文件,就会设置低位字节的高位.pip
        参数指定了要等待的进程.如果省略,当人员子进程退出时,wait()函数就会返回.

    waitpid(pid,options) 等待进程ID位pid的子进程的状态出现变化,就返回包含其进程ID和退出状态只是的一个元组
        (pid,status),编码与wait()函数中相同.options对应常规操作应该是0或者WNOHANG,以避免当没有子进程状态可
        用时出现挂起.此函数还可以收集只为某些原因而停止执行的子进程的相关信息.
        将options设置为WCONTINUED时,可以从一个"停止之后通过任务控制恢复操作的子进程"收集信息.
        将options设置为WUNTRACED,可用从一个"已经停止,但尚在未报告状态信息的子进程"收集信息.

    wait3([options]) 同waitpid(),但此函数将等待所有子进程张的变化.返回(pid,status,rusage).rusage包含
        resource.getrusage()函数返回的资源使用信息.
