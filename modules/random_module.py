...
种子和初始化:(控制基础随机数生成器的状态)
    seed([x]) 初始化随机数生成器. 如果省略x或x为None,则使用系统时间来设置生成器. 如果x是整数或者长整数,则使
        用该值.如果x不是整数,那么它必须是可散列的对象且将hash(x)的值作为种子
...
随机序列:
    ...
    shuffle(x, [random]) 随机原地打乱"列表x"中的项, random是可选参数,指定随机生成函数. 如果提供该参数,则该
        参数是一个无参的函数,且该函数的返回值在[0.0, 1.0)范围内
...
实值随机分布:
    ...
    cunifvariate(mean, arc) 圆形一致分布,mean是平均角, arc是沿平均角周围居中的分布范围. 这些值必须设置在
        0-pi之间的弧度范围内.返回值范围(mean - arc/2, mean + arc/2)
...
注意:
    ...
    3.通过实现random.Random的子类病实现random(),seed(),getstate(),setstate()和jumpahead(), 就可以创建
    新的随机数生成器类型. 实际上,该模块的其他函数在内部都作为Random的方法实现的
    4. 该模块提供了两种生成器类--WichmannHill(早期Python版本所使用的生成器)和SystemRandom(使用系统随机数
    生成器os.urandom()生成随机数).
"""
