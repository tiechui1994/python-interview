...
dump(object, file [, protocol]) 将object的序列化表示转存到文件对象file当中. protocol设置数据输出格式,
    ...
    协议2是较新的协议, 提供了更高效的类和实例化序列; 协议3(默认)是用于Python3.0增加的,并且不向后兼容,支持bytes.
...
Pickler(file, [, protocol]) 创建序列化对象, 利用指定的序列化protocol将数据写入到文件对象file中
    方法:
        dump(object) 序列化存储存储. 在将object转储之后,内存当中会记住它是身份(内存id). 如果接下来使用dump操作
            写入相同的对象(内存id不变),则保存前面已经转储对象的引用,而不是写新的副本. (注意,此时序列化的内容是内存
            当中保存的对象, 对应属性的更改不气任何作用)
