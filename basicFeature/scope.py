...
名称的绑定(确定名称作用域)
    ...
    下面结构将会绑定名称：
        ...
        3.类和函数定义(绑定类或函数的名称于定义它们的代码块中):类和函数定义将类名和函数名作为变量引入当前作用域,类体和函数体将形成另外一个作用域;
        ...
        注: from ... import *形式的import语句绑定导入的模块中定义的所有名称,除了以下划线(_)开头的那些.这种形式只能在模块级别使用.
    ...
    如果一个变量在代码块中只使用但未定义,那么它是一个自由变量.(自由变量的名称解析发生在运行时刻，不是在编译时刻)


名称的解析(确定运行时名称的值)
    作用域定义一个代码块中名称的可见性. 如果一个局部变量在一个代码块中定义,那么它的作用域包括那个代码块;
    如果定义出现在函数代码块中,那么其作用域扩展到这个函数代码块中包含的任何代码块,除非某个被包含的代码块为该名称引入一个不同的绑定.

    当一个名称在代码块中使用时,它使用包含它最近的作用域解析.对于一个代码块所有可见作用域的集合称做代码块的环境.

    异常:
        当一个名称完全找不到是,将引发一个NameError异常.
        如果当前的作用域是一个函数作用域,而且名称引用一个局部变量,这个变量在该名称使用的时候还没有绑定到一个值,则引发一个UnboundLocalError异常(UnboundLocalError是NameError的子类)
    ...
    global的运行机制和作用域规则:
        ...
        名称在顶级命名空间中的解析通过搜索全局命名空间,即包含该代码块的模块的命名空间,和内建的命名空间(模块builtins的命名空间).
        首先查找全局命名空间,如果那里找不到名称,就会查找内建的命名空间.
        global语句必须位于该名称的所有引用之前.

        global语句的作用域与同一代码块中的名称绑定操作相同(局部范围可以操作上层的变量).如果包含自由变量的最内层定义域包含一条global语句,
        那么这个自由变量[声明式变量]被认为是一个全局变量.
