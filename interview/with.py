...
with相关概念:
    ...
    上下文管理器(Context Manager): 支持上下文管理协议的对象，这种对象实现了__enter__()和__exit__()方法.上下文管理器定义执行with
        语句时要建立的运行时上下文,负责执行with语句块上下文中的进入与退出操作.通常使用with语句调用上下文管理器,也可以通过直接调用其
        方法来使用.
    运行时上下文(runtime context)：由上下文管理器创建,通过上下文管理器的__enter__()和__exit__()方法实现,__enter__()方法在语句
        体执行之前进入运行时上下文,__exit__()在语句体执行完后从运行时上下文退出.with语句支持运行时上下文这一概念.
    上下文表达式(Context Expression)：with语句中跟在关键字with之后的表达式,该表达式要返回一个上下文管理器对象.
    语句体(with-body)：with语句包裹起来的代码块,在执行语句体之前会调用上下文管理器的__enter__()方法,执行完语句体之后会执行
        __exit__()方法.
...
with语句执行流程:
    ...
    3. 执行语句体with-body
    4. 不管是否执行过程中是否发生了异常,执行上下文管理器的__exit__()方法,__exit__()方法负责执行"清理"工作,如释放资源等.
    ...
    5. 出现异常时,如果__exit__(type, value, traceback)返回 False,则会重新抛出异常,让with之外的语句逻辑来处理异常,这也是通用做法;
       如果返回 True,则忽略异常,不再对异常进行处理.
"""

"""
自定义上下文管理器:(实现__enter__和__exit__)
    context_manager.__enter__(): 进入上下文管理器的运行时上下文,在语句体执行前调用.with语句将该方法的返回值赋值给as子句中的target,
        如果指定了as子句的话
    context_manager.__exit__(exc_type, exc_value, exc_traceback): 退出与上下文管理器相关的运行时上下文,返回一个布尔值表示是否
        对发生的异常进行处理. 参数表示引起退出操作的异常.
...
"""
contextlib 模块:
    contextlib模块提供了3个对象: 装饰器contextmanager, 函数nested和上下文管理器closing. 用这些对象,可以对已有的生成器函数
    或者对象进行包装,加入对上下文管理协议的支持,避免了专门编写上下文管理器来支持with语句

    contextmanager: 用于对生成器函数进行装饰,生成器函数被装饰以后,返回的是一个上下文管理器,其__enter__()和__exit__()方法由
        contextmanager负责提供. 被装饰的生成器函数只能产生一个值,否则会导致异常RuntimeError;产生的值会赋值给as子句中的target
