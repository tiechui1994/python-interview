...
"""
队列:
    Queue([maxsize]) 创建共享的进程队列. maxsize是队列允许的最大项数. 省略则无大小限制. 底层队列使用管道和锁实现.另外,还需要运行
                     支持线程以便将队列中的数据传输到底层管道当中.
    方法:
        cancel_join_thread() 不会在进程退出时自动链接后台进程. 这可以防止join_thread()方法阻塞.
        close() 关闭队列, 防止队列中加入更多数据. 调用此方法时,后台线程将继续写入那些已入队列但尚未写入的数据.但将在此方法完成时马
                上关闭.如果队列被垃圾回收,将自动调用此方法. 关闭队列不会在队列消费者中产生任何类型是数据结束信号或异常.
        empty() 如果队列为空,则返回True. 如果其他进程或线程正在往队列中添加项,结果是不可靠的.
        full() 如果队列已满,返回True
        get([block, timeout]) 返回队列当中一个项. 如果队列为空,此方法将阻塞,直到队列有项可用为止.block用于控制阻塞行为,默认是True,
                              如果设置为False,将引发Queue.Empty异常. timeout是可选的超时时间,用在阻塞模式当中.
        get_nowait() 等价get(False)
        join_thread() 连接队列的后台线程. 此方法用于在调用close()方法之后,等待所有队列项背心消耗.默认情况下,此方法由不是队列的原始
                      创建者的所有进程调用. 调用cancel_join_thread()方法可以禁止此种行为.
        put(item, [block, timeout]) 将item放入队列. 如果队列已满,此方法将阻塞至有空间可用为止. block控制阻塞行为,默认是True.
        put_nowait(item) 等价于put(item, False)
        qsize() 返回当前队列中项的正确数量. 此函数的结果并不可靠.


    JoinableQueue([maxsize]) 创建可连接的共享进程队列. 是一个Queue对象,但队列允许项的消费者通知生产者项已经被成功处理. 通知进程是
                             使用共享的信号和条件变量实现的.
    方法:
        task_done() 消费者使用此方法发送信号,表示get()返回的项已经被处理. 如果调用此方法的次数大于从队列中删除的项的数量,将引发
                    ValueError
...
管道:
    Pipe([duplex]) 在进程之间创建一条管道,并返回元祖(conn1, conn2),其中conn1和conn2是表示管道两端的Connection对象. 默认情况下,
                   管道是双向的.如果将duplex设置为False,conn1只能用于接收,而conn2只能用于发送.必须在创建和启动使用管道的Process
                   对象之前调用Pipe()方法
    Connection方法:
        ...
        recv_bytes([maxlength]) 接收send_bytes()发送的一条完整的字节消息. 如果进入的消息超过maxlength,引发IOErro,并且在连接上无法
                                进行进一步读取.
        recv_bytes_into(buffer, [offset]) 接收一条完整的字节消息,并把它保存在buffer对象中, 该对象支持可写入的缓存区域接口(即bytearray
                                          对象或者类似的对象), offset指定缓存区中放置消息处的字节位移. 返回值是收到的字节数. 如果
                                          消息长度大于可用的缓存区空间,将引发BufferTooShort
