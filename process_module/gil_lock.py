...
信号量:
    ...
    Semaphone方法:
        ...
        release() 通过内部计数器的值加1来释放一个信号量. 如果计数器是0, 而且另外一个线程正在等待, 该线程会被唤醒.
        如果是多个线程正在等待, 只能唤醒其中的一个线程.

    threading.BoundedSemaphore([value]) 工作方式与Semaphore的一致, 但release()操作次数不能超过acquire()操作
    次数
"""
